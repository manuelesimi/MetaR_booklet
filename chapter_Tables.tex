...
Wherever a table name appears (in a Table node or in an Analysis script, see Chapter ~\ref{chap:Analyses}), the tool can be open to see the rows and columns of that table along with their values. Figure ~\ref{fig:ActivateTableViewer} shows how to activate the tool on a Table node. The first time is activated, the tool appears at the bottom of the MPS UI, as shown in Figure ~\ref{fig:TableViewerInMPS}.
...
\caption[The Table Viewer Tool]{\textbf{The Table Viewer Tool}. The tool displays the content of the selected table. At the top, we can see the name of the table and the number of records loaded. Columns can be resized and rows scrolled for a full visualization of their values.}
\label{fig:ToolVisualizationOptions}
\end{figure}

The tool is immediately available for those tables directly loaded from the file system (e.g. Table nodes) and their references. Other tables are instead created by Analysis scripts. In this latter case, the tool is available after the first script execution (see see Chapter ~\ref{chap:Analyses}). Moreover, if the structure or content of a table change, the script must be executed again in order to reflect such changes in the tool.
