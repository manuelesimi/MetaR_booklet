...
When annotation tables are available in the current model, the user can open the Table to annotate and use the "Use Column Groups From Table: <table name>" intention (\intentionLightBulb) when the cursor is on top of \texttt{Table}.  Sample IDs are matched with the column names and the groups listed in the same row are attached to the Column. In addition, if the groups are not defined in the ColumnGroupContainer, they are created in the annotation process. The Column Group Container is also created if it does not exist.

To see how they work in practice, Figure ~\ref{fig:AnnotateTableSample} shows the content of a sample annotation table that can be used to create the same groups shown in Figure ~\ref{fig:ExampleTable}.
...
As first step, we create a corresponding Table node in the model (Figure ~\ref{fig:NewAnnotationTable}).   
\begin{figure}[h!tbp]
  \centering
...
Then, we invoke the ... intention as shown in Figure ~\ref{fig:AnnotateTableIntention}
\begin{figure}[h!tbp]
  \centering
...
These steps will result in the same annotated table shown in Figure ~\ref{fig:ExampleTable}.